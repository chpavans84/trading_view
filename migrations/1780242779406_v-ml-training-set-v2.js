/**
 * Migration: v_ml_training_set v2 — adds Phase B5 Iteration features
 *
 * Replaces the prior v_ml_training_set view with one that includes:
 *   - vix_close, vix_above_20         (market regime gate — biggest miss in v1)
 *   - spy_5d_chg                      (broader market momentum)
 *   - day_of_week                     (0=Sun … 6=Sat; Wed/Thu had +6-8pp win edge)
 *   - is_wed, is_thu                  (one-hot for the two best days)
 *   - market_cap_band                 (ordinal: 1=mega 2=large 3=mid 4=small 5=micro)
 *   - sector one-hot                  (sect_tech … sect_re — 11 cols total)
 *   - uw_flow_bullish_pct_sq          (quadratic — mixed flow outperforms extremes)
 *   - uw_flow_bullish_centered        (linear part centered at 50)
 *
 * Why all of this: deep research on 60-day backtest showed:
 *   - Win rate varies 5%-71% by sector → sector dummies critical
 *   - Mid-cap = 76.9% win, Mega = 60.3% → mcap band matters
 *   - Wed/Thu = 66%, Tue = 54% → DOW matters
 *   - Mixed bullish flow (40-60%) outperforms strong bullish (>60%) → non-linear
 *
 * Phase 1 target: AUC 0.582 → 0.62+, win rate 51% → 65%.
 */
...
export const up = (pgm) => {
  ...
  pgm.sql(`
    ...
    SELECT
      ...
      -- NEW: Day-of-week (Wed=3, Thu=4 are best per analysis)
      ...
      -- NEW: Market cap band (1=mega 2=large 3=mid 4=small 5=micro 0=unknown)
      CASE
        WHEN w.market_cap_usd >= 200e9 THEN 1
        WHEN w.market_cap_usd >= 10e9  THEN 2
        WHEN w.market_cap_usd >= 2e9   THEN 3       -- mid-cap: 76.9% win in backtest
      ...
      -- NEW: Sector one-hot (11 dummies — captures per-sector intercepts)
      ...
      -- NEW: Quadratic UW bullish term (peaks at 50% = mixed flow)
      -- centered: bullish_pct - 50 (negative when bearish, positive when bullish)
      (COALESCE(w.uw_flow_bullish_pct, 0) - 50)                            AS uw_bull_centered,
      -- squared distance from 50: 0 at 50, 2500 at 0 or 100 (penalizes extremes)
