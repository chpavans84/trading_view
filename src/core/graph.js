...
export async function setupSchema() {
  const statements = [
    ...
    'CREATE CONSTRAINT index_name     IF NOT EXISTS FOR (i:Index)   REQUIRE i.name   IS UNIQUE',
    'CREATE CONSTRAINT etf_ticker     IF NOT EXISTS FOR (e:ETF)     REQUIRE e.ticker IS UNIQUE',
...
export async function upsertCompany(data) {
  const {
    ...
    exchange     = null, country       = null,
    market_cap_usd = null, employees   = null,
    founded_year = null, description   = null,
  } = data;
  await run(`
    ...
    SET c.name           = $name,
        ...
        c.last_updated   = date()
  `, { ticker, name, sector, sub_sector, exchange, country,
       market_cap_usd, employees, founded_year, description });
}
...
export async function upsertRelationship({ from, to, type, props = {} }) {
  ...
  const setParts = Object.keys(props).map(k => `r.${k} = $props.${k}`);
  setParts.push('r.last_updated = date()');
...
export async function getContagionImpact(ticker, eventPct = -10, maxHops = 3) {
  ...
  const records = await run(`
    ...
    RETURN impact.ticker  AS ticker,
           impact.name    AS name,
           impact.sector  AS sector,
           hops,
           [r IN rels | {
             type:   type(r),
             weight: coalesce(r.market_share_pct, r.revenue_pct, r.ownership_pct, r.overlap_pct, 30)
           }] AS rel_chain
    ORDER BY hops ASC
  `, { ticker });
  ...
  return records.map(r => {
    ...
    const impactPct    = isCompetitor ? -rawImpact : rawImpact;
...
export async function getSympathyTrades(ticker) {
  const records = await run(`
    ...
    RETURN peer.ticker AS ticker,
           ...
           type(r)     AS rel_type,
           coalesce(r.overlap_pct, r.market_share_pct, r.revenue_pct, 30) AS strength
...
export async function getSystemicRisk(ticker) {
  ...
  return {
    ticker,
    connections: conn,
    rel_types:   records[0].get('rel_types'),
...
export async function getCompanySubgraph(ticker, hops = 2) {
  const [nodeRecords, edgeRecords] = await Promise.all([
    run(`
      MATCH (origin:Company {ticker: $ticker})-[*1..${hops}]-(c:Company)
      RETURN DISTINCT c.ticker AS ticker, c.name AS name, c.sector AS sector
    `, { ticker }),
    run(`
      MATCH (a:Company {ticker: $ticker})-[r*1..${hops}]-(c:Company)
      WITH a, c, relationships(r)[0] AS rel
      RETURN a.ticker AS from, c.ticker AS to, type(rel) AS type,
             coalesce(rel.market_share_pct, rel.revenue_pct, rel.overlap_pct, 30) AS weight
    `, { ticker }),
  ]);

  return {
    center: ticker,
    nodes:  nodeRecords.map(r => ({ id: r.get('ticker'), name: r.get('name'), sector: r.get('sector') })),
    edges:  edgeRecords.map(r => ({ from: r.get('from'), to: r.get('to'), type: r.get('type'), weight: r.get('weight') })),
  };
}
...
export async function getGraphStats() {
  ...
  return {
    companies:     records[0].get('companies').toNumber(),
    relationships: records[0].get('relationships').toNumber(),
  };
}
