...
const RELATIONSHIPS = [
  { from: 'ASML', to: 'TSM',     type: 'SUPPLIES_TO',     props: { product_category: 'EUV Lithography',       market_share_pct: 90, supply_criticality: 'sole',    can_substitute: false, source: 'annual_report', confidence: 'high'   } },
  { from: 'ASML', to: 'SAMSUNG', type: 'SUPPLIES_TO',     props: { product_category: 'EUV Lithography',       market_share_pct: 90, supply_criticality: 'sole',    can_substitute: false, source: 'annual_report', confidence: 'high'   } },
  { from: 'ASML', to: 'INTC',    type: 'SUPPLIES_TO',     props: { product_category: 'EUV Lithography',       market_share_pct: 90, supply_criticality: 'primary', can_substitute: false, source: 'annual_report', confidence: 'high'   } },
  ...
  { from: 'SAMSUNG', to: 'QCOM', type: 'MANUFACTURES_FOR', props: { revenue_pct: 10, product: 'Snapdragon (secondary)',   source: 'news', confidence: 'medium' } },
  ...
  { from: 'SAMSUNG', to: 'NVDA', type: 'SUPPLIES_TO',     props: { product_category: 'HBM3e Memory', market_share_pct: 50, source: 'earnings_call', confidence: 'high' } },
...
export async function seedGraph() {
  ...
  for (const r of RELATIONSHIPS) {
    ...
    catch (e) { console.warn(`[graph-seed] Skipped ${r.from}→${r.to}: ${e.message}`); failed++; }
  }
